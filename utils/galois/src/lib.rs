...
fn slice_xor(input: &[u8], out: &mut [u8]) {
    assert_eq!(input.len(), out.len());
    input.iter().zip(out).for_each(|(i, o)| *o ^= *i);
}
...
pub fn mul_slice(c: u8, input: &[u8], out: &mut [u8]) {
    ...
    let bytes_done: usize = unsafe { reedsolomon_gal_mul(low, high, input_ptr, out_ptr, size) };
...
mod tests {
    ...
    use {super::*, alloc::vec, quickcheck::quickcheck};
    ...
    fn fill_random(v: &mut [u8]) {
        getrandom::getrandom(v).unwrap();
    }
