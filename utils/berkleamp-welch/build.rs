...
fn gen_inv_table(log_table: &[u8; FIELD_SIZE], exp_table: &[u8; EXP_TABLE_SIZE]) -> [u8; 256] {
    let mut result: [u8; 256] = [0; 256];

    for i in 1..FIELD_SIZE {
        let log = log_table[i] as usize;
        let exp = exp_table[FIELD_SIZE - log - 1];
        result[i] = exp;
    }
...
macro_rules! write_table {
    ...
    (2D => $file:ident, $table:ident, $name:expr, $type:expr) => {{
        ...
        let mut table_str =
            String::from(format!("pub static {}: [[{}; {}]; {}] = [", $name, $type, cols, rows));
...
fn write_tables() {
    ...
    let inv_table = gen_inv_table(&log_table, &exp_table);
